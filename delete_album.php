...
<body>
	...
	<div class="jumbotron">
		...
		<form method="POST">
			<?php
				$query = $bdd -> query("SELECT a_slug, idAlbum, name FROM album WHERE idUser='".$_SESSION['idUser']."'");
				while($run = $query -> fetch(PDO::FETCH_OBJ)) {
					$album_id = $run -> idAlbum;
					$album_name = $run -> name;
					$a_slug = $run -> a_slug;
					$idUser = $_SESSION['idUser'];
					$query1 = $bdd -> query("SELECT * FROM picture WHERE idAlbum = $album_id");
					...
					if(!$run1) {
						...
						$p_slug = $run1 -> p_slug;
						$src_img = 'uploads/' . $idUser . '/' . $a_slug . '/' . $p_slug;
					}
			...
			<div id='view_box'>
				...
				?>
				<label for="<?php echo $album_id ?>">
				<img src='uploads/<?php echo $idUser,"/", $a_slug, "/Main/", $p_slug ?>' height=180 alt="No picture for present." />
				<br/>
				<input id="<?php echo $album_id ?>" type="checkbox" name="album_delete[]" value="<?php echo $album_id ?>">
			...
			<?php
				if(isset($_REQUEST['album_delete'])) {
					foreach($_POST['album_delete'] as $album_delete) {
						$idUser = $_SESSION['idUser'];
						$query = $bdd -> query("SELECT a_slug FROM album WHERE idAlbum = $album_delete");
						$a_slug = $query -> fetch(PDO::FETCH_OBJ) -> a_slug;
						$delete = $bdd -> exec("DELETE FROM album WHERE idAlbum = $album_delete");
						delDir('uploads/' . $idUser . '/' . $a_slug);
					}
				}
				function delDir($dirName) {
						...
						while(false !== ($item = readdir($handle))) {
							if($item != "." && $item != "..") {
								if(is_dir("$dirName/$item")) {
									...
									if(unlink("$dirName/$item"))echo "Picture Deleted： $dirName/$item<br /> ";
								}
							}
						}
					   closedir($handle);
					...
					if(rmdir($dirName)) {
						echo "<script>alert('Delete album successefully!')</script>";
